...
def worker(gpu, cfg):
        ...
        if step == cfg.num_steps or step % cfg.save_ckp_interval == 0 or step == resume_step:
            ...
            if cfg.rank == 0:
                if cfg.use_textInversion and not cfg.freeze_text_embedding:
                    ...
                    local_embedding_path = osp.join(cfg.log_dir, f'embeddings/{cfg.embedmanager.placeholder_strings[0]}_{step:04d}.pth')
                ...
                if cfg.train_adapter:
                    local_model_path = osp.join(cfg.log_dir, f'checkpoints/adapter_{step:04d}.pth')
    ...
    if cfg.rank == 0:
        ...
        id_name = exp_name.split("_")
        logging.info('Congratulations! The training is completed!')
        source_path = f'workspace/dreamvideo/subjectLearningStep1/{id_name[0]}_step1/embeddings/{id_name[0]}_3000.pth'
        destination_path = f'workspace/dreamvideo/concept_embeddings/'
        if os.path.exists(source_path):
            shutil.move(source_path, destination_path)
            source_path = destination_path + f'{id_name[0]}_3000.pth'
            destination_path = destination_path + f'{id_name[0]}.pth'
            os.rename(source_path, destination_path)

        source_path = f'workspace/dreamvideo/subjectLearningStep2/{id_name[0]}_step2/checkpoints/adapter_1000.pth'
        destination_path = f'workspace/dreamvideo/concept_lora/'
        if os.path.exists(source_path):
            shutil.move(source_path, destination_path)
            source_path = destination_path + f'adapter_1000.pth'
            destination_path = destination_path + f'{id_name[0]}.pth'
            os.rename(source_path, destination_path)

        source_path = f'workspace/dreamvideo/motionLearning/{id_name[0]}_step2/checkpoints/adapter_1000.pth'
        destination_path = f'workspace/dreamvideo/concept_lora/'
        if os.path.exists(source_path):
            shutil.move(source_path, destination_path)
            source_path = destination_path + f'adapter_1000.pth'
